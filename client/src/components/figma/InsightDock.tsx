...
export function InsightDock({ className, onFiltersChange, onJumpTo }: InsightDockProps) {
  ...
  return (
    <div className={cn("h-full w-80 xl:w-80 bg-meld-canvas border-l border-gray-200 flex-shrink-0", className)}>
      <div className="p-4 xl:p-5 h-full flex flex-col">
              ...
              {entryTypes.map((type) => {
                ...
                return (
                  <Button
                    ...
                    className={cn(
                      "text-xs border-meld-graysmoke hover:border-meld-sand transition-colors flex items-center gap-1.5 xl:gap-2",
                  ...
                  >
                    ...
                    <span className="xl:hidden">{type.label.split(' ')[0]}</span>
                  </Button>
                );
              })}
            </div>
        ...
        <div className="space-y-4 mb-8">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-1 gap-1">
              {entryTypes.map((type) => (
          ...
          <div className="space-y-2">
            ...
            {daysUntilNarrativeCheckIn <= 5 && (
              ...
              >
                ...
                <span className="hidden xl:inline">Next Narrative Check-in (due in {daysUntilNarrativeCheckIn} days)</span>
                <span className="xl:hidden">Narrative Check-in ({daysUntilNarrativeCheckIn}d)</span>
              </Button>
            )}
        ...
        {(selectedTypes.length > 0 || selectedValueTag || searchQuery) && (
          <div className="mt-6 pt-4 border-t border-meld-ink/20">
            ...
            <div className="flex flex-wrap gap-1">
              {selectedTypes.map(type => (
                <span key={type} className="text-xs bg-meld-graysmoke/50 text-meld-ink px-2 py-1 rounded">
                  {entryTypes.find(t => t.id === type)?.label.split(' ')[0]}
                </span>
              ))}
              {selectedValueTag && (
                <span className="text-xs bg-meld-graysmoke/50 text-meld-ink px-2 py-1 rounded">
                  {selectedValueTag}
                </span>
              )}
              {searchQuery && (
