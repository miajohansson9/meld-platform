...
export function MorningSegment({ className }: MorningSegmentProps) {
  ...
  const handleComplete = () => {
    if (!focusIntention.trim()) {
      showToast({
        message: "Please share what will move you forward today.",
        status: 'error'
      });
      return;
    }
    ...
    if (alignment === null) {
      showToast({
        message: "Please check your alignment with yesterday's intention.",
        status: 'error'
      });
      return;
    }
    ...
    showToast({
      message: "Daily Compass saved — you'll see how it shapes your North Star in Log tonight.",
      status: 'success'
    });
  ...
  const handleSkip = () => {
    showToast({
      message: "Your next prompt will appear tomorrow morning.",
      status: 'success'
    });
  };
  ...
  return (
    <div className={cn("bg-white rounded-lg border border-meld-graysmoke/50 overflow-hidden", className)}>
      ...
      <div className="p-6 space-y-6">
        ...
        {/* Alignment Check */}
        <div className="space-y-3">
          <label className="text-sm font-medium text-meld-ink">How aligned were you with yesterday's intention?</label>
          <p className="text-xs text-meld-ink/60 mb-3">
            Yesterday: "delegate sprint brief"
          </p>
